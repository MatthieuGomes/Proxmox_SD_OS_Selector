...
# Etape 2: Copie des icônes et templates
...
# Etape 3: Création du fichier create.sh
...
printf "$STEP/$MAX_STEP Reussie !\n\n"
STEP=$((STEP+1));
# Étape 4: Télécharger et installer le package
...
printf "$STEP/$MAX_STEP Reussie !\n\n"
STEP=$((STEP+1));
# Étape 5: Créer un fichier .rules dans /etc/udev/rules.d/
printf "$STEP/$MAX_STEP : Création du fichier udev rules...\n"
cp $PWD/files/99-streamdeck.rules $UDEV_RULES_FILE

printf "$STEP/$MAX_STEP Reussie !\n\n"
STEP=$((STEP+1));
# Étape 6: Changer le propriétaire de /dev/uinput afin d'éviter les messages d'errreurs
